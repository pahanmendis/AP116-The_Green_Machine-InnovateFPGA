...
def create_items(container,data):
    ...
    initial = 20220403000050
    for i in range(len(data)):
        reading = get_sensor_data(str(initial+i),data[i])
...
def get_sensor_data(item_id,data_item):
    print(item_id," :", end = " ")
    print(data_item)
    reading1 = {'id' : item_id,
            'machineId' : 'Machine1',
            'env_temp' : float(data_item[2]),
            'env_hum' : float(data_item[1]),
            'light' : int(data_item[3]),
            'co2' : int(data_item[0]),
            'soil_moisture' : int(data_item[4]),
            'soil_temp' : random.uniform(29,35),
            'soil ph': random.uniform(4,10)
            }
